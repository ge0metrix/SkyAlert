...
class PlaneWatcherApp(App):
    ...
    def on_mount(self) -> None:
        ...
        columns = ["Hex", "Type", "Reg", "Flight", "Closest (nm)", "First Seen", "Last Seen", "Helicopter?", "Interesting?"]
    ...
    def refresh_data(self) -> None:
        ...
        for ac in sortedac:
            ...
            if ac.is_interesting and ac.is_helicopter:
                seentable.add_row(f'[red on yellow]{ac.hex}[/red on yellow]', f'[red on yellow]{ac.type}[/red on yellow]', f'[red on yellow]{ac.tail}[/red on yellow]', f'[red on yellow]{ac.flight}[/red on yellow]', f'[red on yellow]{closest}[/red on yellow]', f'[red on yellow]{first_seen}[/red on yellow]', f'[red on yellow]{last_seen}[/red on yellow]', f'[red on yellow]{ac.is_helicopter}[/red on yellow]', f'[red on yellow]{ac.is_interesting}[/red on yellow]', key=ac.hex)
            elif ac.is_interesting:
                seentable.add_row(f'[red]{ac.hex}[/red]', f'[red]{ac.type}[/red]', f'[red]{ac.tail}[/red]', f'[red]{ac.flight}[/red]', f'[red]{closest}[/red]', f'[red]{first_seen}[/red]', f'[red]{last_seen}[/red]', f'[red]{ac.is_helicopter}[/red]', f'[red]{ac.is_interesting}[/red]', key=ac.hex)
